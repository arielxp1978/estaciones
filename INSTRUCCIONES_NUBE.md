...
3. En el **SQL Editor**, ejecuta lo siguiente para crear la tabla (o usa `drop table beneficios;` si quieres borrarla y empezar de cero):
   ...
   create table if not exists beneficios (
     ...
     estacion text, -- Mantener nombre original
     banco text,
     medio_pago text,
     dia text,
     combustible text,
   ...
   -- Políticas para 'beneficios': Lectura pública, escritura prohibida para anónimos
   create policy "Permitir lectura pública de beneficios" on beneficios for select using (true);
   
   -- Políticas para 'analisis_ia': Lectura pública
   create policy "Permitir lectura pública de analisis" on analisis_ia for select using (true);

   -- NOTA: El script de GitHub usa la 'service_role key', que salta el RLS automáticamente
   -- para poder insertar y borrar datos.
   ```
4. Ve a **Project Settings > API** y copia la `URL` y la `service_role key` (esta ÚLTIMA es la que debes poner en GitHub Secrets como `SUPABASE_KEY`).
