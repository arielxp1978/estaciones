...
import { 
  Moon, Sun, CheckCircle2, Sparkles, Loader2, Fuel, 
  Lock, ArrowRight, MessageSquare, X, Send, ThumbsUp, Star, LayoutDashboard, ChevronLeft, LogOut, AlertCircle
} from 'lucide-react';
...
const App = () => {
  ...
  const generateMarkdownTable = (brand, items) => {
    ...
    selectedFuels.forEach(fuel => {
      ...
      md += `## Combustible: ${fuel}\n\n`;
      if (sorted.length === 0) md += `> Sin promociones vigentes.\n\n`;
      else {
  ...
  const handleAiConsult = async () => {
    ...
    if (!geminiApiKey) { setAiResponse("⚠️ Revisa la clave API en Netlify."); return; }
    setIsAiConsulting(true);
    setAiResponse(null);
    const context = files.map(f => `MARCA ${f.brand}:\n${f.items.map(i => `- ${i.banco}: ${i.descuento}% desc el ${i.dia}`).join('\n')}`).join('\n\n');
    try {
      const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${geminiApiKey}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          contents: [{ parts: [{ text: `Asesor ahorro combustible Argentina. Tablas PRIORIDAD. Directo.\n\nDatos:\n\n${context}\n\nConsulta: "${userPrompt}".` }] }]
        })
      });
      const data = await response.json();
  ...
  const handleSendFeedback = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const errJson = await response.json().catch(() => ({ message: "Error de servidor (RLS?)" }));
        setFeedbackError(`Error: ${errJson.message || "No se pudo guardar"}`);
      }
    } catch (e) {
      setFeedbackError("Error de red. Verifica Supabase.");
    }
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 font-sans ${isDarkMode ? 'bg-slate-950 text-white' : 'bg-slate-50 text-slate-900'}`}>
            ...
            <div className={`rounded-[2.5rem] border shadow-2xl overflow-hidden min-h-[50vh] ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-100'}`}>
              <div className="h-full p-8 md:p-14 overflow-y-auto custom-scrollbar">
                <div className="markdown-body prose max-w-none dark:prose-invert overflow-hidden">
                    <div className="table-responsive" dangerouslySetInnerHTML={{ __html: marked.parse(activeFileId ? generateMarkdownTable(activeBrandData?.brand, activeBrandData?.items) : generateLocalRanking()) }} />
                </div>
              </div>
            </div>
            ...
            <div className={`rounded-[2.5rem] border p-8 md:p-10 transition-all ${isDarkMode ? 'bg-slate-900/50 border-slate-800' : 'bg-blue-50/50 border-blue-100 shadow-inner'}`}>
                {!feedbackSent ? (
                    <div className="flex flex-col gap-6">
                        <div className="flex items-center gap-2 text-blue-600 font-black uppercase italic text-lg tracking-tighter"><ThumbsUp size={20} /> ¿Cómo fue tu experiencia?</div>
                        <div className="flex items-center gap-2">
                            {[1, 2, 3, 4, 5].map((star) => (
                        ...
                        <textarea value={feedbackText} onChange={(e) => setFeedbackText(e.target.value)} placeholder="Tu opinión ayuda a mejorar..." className={`w-full p-4 rounded-2xl text-sm outline-none border-2 transition-all resize-none h-24 font-medium ${isDarkMode ? 'bg-slate-900 border-slate-800 focus:border-blue-600 text-white' : 'bg-white border-slate-200 focus:border-blue-600 shadow-sm'}`} />
                        {feedbackError && <div className="text-red-500 text-xs font-black uppercase flex items-center gap-2"><AlertCircle size={14}/> {feedbackError}</div>}
                        <button onClick={handleSendFeedback} disabled={isSendingFeedback || !feedbackText.trim() || feedbackRating === 0} className="bg-blue-600 hover:bg-blue-700 text-white font-black py-4 rounded-2xl shadow-xl transition-all uppercase tracking-widest text-[11px] disabled:opacity-50 flex items-center justify-center gap-2">
                            {isSendingFeedback ? <Loader2 className="animate-spin" size={16} /> : <Send size={16} />} Enviar Feedback
                        </button>
                    </div>
                ) : (
                    <div className="text-center py-6 animate-in zoom-in-95 duration-500"><CheckCircle2 size={48} className="mx-auto mb-4 text-emerald-500" /><h3 className="text-xl font-black uppercase italic tracking-tighter text-emerald-600">¡Recibido!</h3><button onClick={() => setFeedbackSent(false)} className="mt-4 text-[9px] uppercase font-black text-blue-600 hover:underline">Enviar otro</button></div>
                )}
            </div>
          </>
        ) : (
          <div className="animate-in slide-in-from-right-4 duration-500">
            <h2 className="text-2xl font-black italic tracking-tighter uppercase mb-8 border-b pb-4 border-slate-100 dark:border-slate-800">Feedback Privado</h2>
            {isLoadingFeedback ? <div className="flex justify-center py-20"><Loader2 className="animate-spin text-blue-600" size={48}/></div> : (
              <div className="flex flex-col gap-4">
                {allFeedback.map((item) => (
                  <div key={item.id} className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-100 dark:border-slate-800 shadow-xl shadow-slate-200/10">
                    ...
                    <p className="text-sm font-medium leading-relaxed italic">"{item.texto}"</p>
                  </div>
                ))}
              </div>
            )}
      ...
      <style>{`
        ...
        .table-responsive { width: 100%; overflow-x: auto; -webkit-overflow-scrolling: touch; margin: 1.5rem 0; border-radius: 1.5rem; border: 1px solid #e2e8f0; }
